...
- ソフトウエアの改変や公開は、自由にご活用してください。公開する際には、Twitterにてご連絡いただければ幸いです。
...
その結果、ある程度の改善がみられましたので新バージョンでリリースしました。   
その他にも、ネットワークのデータ送受信中やキー押下後にも、再起動が発生していて解決できていないものもあります。
...
・SD直下の "manual.txt" のファイルを表示しています。  
・説明書サンプルを、Github のSAMPLEフォルダに置きました。SDにコピーして使用してください。
