...
・ ＠Robo8080さん開発の「Aiスタックチャン」「Aiスタックチャン2」とできる限り互換性をもてるようしています。
**対応するスマホアプリ等は、一部を除き、ほとんどそのまま使用できます。**
なお、APIの設定に関しては、Ver1.09より独自仕様としましたのでアプリでは設定できません。
SD直下の"startup.json"ファイルを直接書き換えるか 下記のWEBから設定してください。

・ **AiStackChanExの全ての機能をWEBの画面より簡単に使用することができます。**　　
