...
- SDの相性問題やファイル破損により読み取りができない場合の確認方法　
...
なお、現状 "apikey.txt" と　"wifi.txt"ファイルが読み取りできない場合には直接にはわかりません。   
次の方法で設定が正しくされているか確認ができます。
...
もう一つの場合には、上記コマンド(sysInfo)で表示される、「openAiApiKey」 が正しく表示されているか、  
無効となっていないか確認することで対策できます。
...
１個限定の　role-system-content を作成します。何も入力した場合には、ロール設定を初期情報にします。今までの、roleの替わりに使用できます。
