...
// *** 頭頂部にタッチするとSERVO Home/Move の切替え *******
  ...
  // Timer Operation
  if (EX_TIMER_STARTED || EX_TIMER_GO_GET)
  {
    EX_TimerOperation();
  }

  // *** (BtnA) RandomSpeak Start/Stop ***
  if (M5.BtnA.wasPressed() || EX_RANDOM_SPEAK_ON_GET)
  {
    if (!EX_SYSINFO_DISP)
    {
      EX_tone(1);
